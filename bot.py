...
async def provide_recommendations(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    Основываясь на данных и фотографии, дай рекомендации в зависимости от ответов на вопросы про рацион, гормональные изменения, тип кожи, чувствительность и питьевой режим и также дай рекомендацию по уходу за кожей, включая основной уход и дополнительный, также выбери 3 различных наименования средств в каждой категории, доступных в регионе пользователя. Учти время года и погоду в регионе.
    ...
    В конце добавь рекомендации по уходу за кожей на основе ответов.
    """
    ...
    try:
        ...
        await update.message.reply_text(
            "Здорово, что вы заботитесь о себе! Это хорошая возможность чувствовать себя лучше.\n"
            f"{recommendations}\n"
