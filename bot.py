...
async def provide_recommendations(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    Основываясь на данных и фотографии, дай рекомендации в зависимости от ответов на вопросы про рацион, гормональные изменения, тип кожи, чувствительность и питьевой режим и также дай рекомендацию по уходу за кожей. Разработай предложенный домашний уход на основе стандартных дерматологических рекомендаций для лечения и ухода за кожей с типом кожи и спецификой пользователя. Включи основной уход и дополнительный, также выбери 3 различных наименования средств, популярных и доказанных в каждой категории, если есть явные лидеры то сделай пометку как популярное и эффективное средство. Учти время года и погоду в регионе в момент запроса.
    В начале ответа похвали пользователя за заботу о себе.
    Оцени состояние кожи на фотографии, укажи наличие воспалений, комедонов или других проблем.
    Всегда обращайся к пользователю во втором лице ("вы").
    твой ответ не должен содержать призыва задать дальше вопрос.
    """
    ...
    try:
        response = openai.ChatCompletion.create(
            model="gpt-3.5-turbo",
            messages=[
                {"role": "system", "content": "You are a helpful assistant and dermatology expert. Always address the user in the second person ('you')."},
        ...
        await update.message.reply_text(
            ...
            f"{recommendations}\n"
            "Учтите, что рекомендуемые продукты могут быть доступны в различных аптеках, косметических магазинах или онлайн-платформах. "
            "Пожалуйста, проверьте их совместимость с вашим типом кожи перед покупкой и на аллергены.\n\n"
            "Если вы хотите оставить обратную связь, напишите @hurmalo"
        )
